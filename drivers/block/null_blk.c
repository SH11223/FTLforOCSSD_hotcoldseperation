...
static void null_zone_exit(struct nullb_device *dev)
{
	kfree(dev->zones);
}
...
static void null_zone_fill_rq(struct nullb_device *dev, struct request *rq,
			   unsigned int zoffset, unsigned int zleft)
{
	...
	bio_for_each_segment(bvec, rq->bio, iter) {
		...
		zfill = bvec.bv_len / sizeof(struct blk_zone);
		...
		zfill = min_t(unsigned int, zfill, zleft);

		memcpy(addr, &dev->zones[zoffset],
				zfill * sizeof(struct blk_zone));
...
static blk_status_t null_zone_report(struct nullb *nullb,
				     struct nullb_cmd *cmd)
{
	struct nullb_device *dev = nullb->dev;
	struct request *rq = cmd->rq;
	unsigned int ztotal = dev->size / dev->zone_size;
	unsigned int zoffset = blk_rq_pos(rq) /
					(dev->zone_size << ZONE_SIZE_SHIFT);
	unsigned int zleft = ztotal - zoffset;
	unsigned int zcmdmax = (blk_rq_bytes(rq) / sizeof(struct blk_zone)) - 1;

	zleft = min_t(unsigned int, zcmdmax, zleft);

	null_zone_fill_rq(nullb->dev, rq, zoffset, zleft);

	return BLK_STS_OK;
}

static void null_zone_write(struct nullb_cmd *cmd)
{
	struct nullb_device *dev = cmd->nq->dev;
	struct request *rq = cmd->rq;
	sector_t sector = blk_rq_pos(rq);
	unsigned int zidx, rq_sectors;
	struct blk_zone *zone;

	zidx = sector / (dev->zone_size << ZONE_SIZE_SHIFT);
	zone = &dev->zones[zidx];
	rq_sectors = blk_rq_sectors(rq);

	if (zone->cond == BLK_ZONE_COND_IMP_OPEN) {
		u64 last_sect = zone->start + zone->len;

		if (zone->wp == blk_rq_pos(rq)) {
			if (zone->wp + rq_sectors < last_sect) {
				zone->wp += rq_sectors;
			} else if (zone->wp + rq_sectors == last_sect) {
				zone->cond = BLK_ZONE_COND_CLOSED;
				zone->wp += rq_sectors;
			} else { /* zone->wp + rq_sectors > last_sect... */
...
static void null_zone_reset(struct nullb_cmd *cmd)
{
	struct nullb_device *dev = cmd->nq->dev;
	struct request *rq = cmd->rq;
	unsigned int zidx = blk_rq_pos(rq) /
					(dev->zone_size << ZONE_SIZE_SHIFT);
	struct blk_zone *zone = &dev->zones[zidx];
...
static int null_zone_init(struct nullb_device *dev)
{
	unsigned int ztotal = dev->size / dev->zone_size;
	unsigned int zsects = dev->zone_size << ZONE_SIZE_SHIFT;
	unsigned int i;
	sector_t sector = 0;

	dev->zones = kcalloc(ztotal, sizeof(struct blk_zone), GFP_KERNEL);
	if (!dev->zones)
		return -ENOMEM;
